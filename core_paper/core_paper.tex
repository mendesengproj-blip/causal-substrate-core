...
places where it demonstrably does not deliver, and the ten open frontiers. The one
combinatorial flank our theorems had left open---rank-frustrated (pentagonal) loop
lattices---we settle here by explicit construction: such orders exist, but only as rigid
crystals, outside every invariance class and excluded from the Lorentz-invariant
sprinkling by the same non-compactness that drives the valence divergence, so the
boundary theorem is untouched and in fact tightened.
\end{abstract}
...
\noindent\textbf{The flank, settled by construction.} Theorem~\ref{thm:T1} governs
balanced (square) generators only. A homogeneous, finite-valence order whose cycle space
is generated by \emph{rank-frustrated} cycles (pentagons, $\varphi=\pm1$) and percolates
would escape all three legs. Such orders \emph{exist} [theorem, this work]: three
explicit exemplars---a $\mathbb{Z}^2$ crystal with five generators, and the one-relator
groups $\langle a,b,c\mid aba{=}bc\rangle$ and $\langle a,b,c,d,e\mid abc{=}ed\rangle$
whose Cayley graphs are vertex-transitive, locally finite, of finite valence, non-graded,
with pentagon-generated percolating cycle space (girth and non-generation verified by
finite permutation-quotient certificates). The construction corrects the intuition it was
built to test: rank frustration does not compete with finite valence---it competes with
\emph{statistical invariance}. Every exemplar is a rigid deterministic crystal, and no
crystal is Lorentz-invariant~\cite{BHS2009}, while layer~1 forbids finite valence for any
invariant rule on the sprinkling. The flank is therefore inhabited, but lies strictly
outside the invariant class: the boundary theorem is not weakened but sharpened, and its
pentagonal escape merges with the single genuine frontier that remains, non-Poisson
measures (Sec.~\ref{sec:open}, item 1). Two facts frame the exemplars: every
foliated/CDT-family substrate uses squares, so a percolating pentagonal substrate is new
physics by itself; and percolating rank frustration is precisely the profile of
sprinklings, where valence diverges---now understood as the mechanism, not a coincidence,
by which invariance and finite valence exclude each other.
...
\emph{hosted, not selected}: replacing $SU(3)$ by $SU(4)$---and by the exceptional
$G_2$---reproduces the full phenomenology (ordering at finite $J$; gapless Goldstones
counting $\dim G$, the sequence $8\to15\to14$; a protected integer topological charge;
confinement with decreasing $\sigma(\beta)$) [measured]---the ``3'' is the world's
choice, not the substrate's. $G_2$ is the sharp control: exceptional, with a \emph{real}
fundamental and \emph{trivial center}, it still mirrors $SU(N)$ at matched coupling
$u=\beta/2N^2$, so the measured confinement signature (Creutz ratio, rising static
potential, $\sigma$ falling with $\beta$) is \emph{center-independent at the accessible
lattice scales}---the asymptotic string breaking that distinguishes a trivial center lies
beyond reach here and is not claimed [measured, pre-registered]. The Pontryagin index
tracks the Dynkin index of the embedding (the estimator reads $2T(R)\times$ degree),
another discrete invariant fixed by group theory rather than by $N$. Second, existence is
...
\# & Frontier & Status \\
\colrule
1 & pentagonal loop lattices & resolved by construction: they exist but only as rigid
crystals (non-invariant); the Lorentz-invariant version is excluded by layer~1 and BHS \\
...
Table~\ref{tab:open} is the boundary of the theory as of this writing. Item 1, the
constructive escape the combinatorial theorems of Sec.~\ref{sec:trichotomy} once left
open, is now closed inside the invariant class (its only realization is a non-invariant
crystal); items 2 and 4 are where new physics would enter if it enters; items 5--7 are
the technical debts; items 8--10 are the declared scope walls.
...
an emergent photon on an invariant causal substrate must falsify Lemma~\ref{lem:pair} or
leave the Poisson measure---the pentagonal-flank route through
Theorems~\ref{thm:T1}--\ref{thm:T4} is now closed by construction, its only realization
being a non-invariant crystal; emergent chirality must break Lemma~\ref{lem:isometries} or
...
\bibitem{WenComplement} M. Alves Mendes, \emph{A classification theorem for emergent
$U(1)$ on causal substrates: string-net admissibility coincides with broken Lorentz
invariance}, companion paper, submitted to Found. Phys. (2026); source at
