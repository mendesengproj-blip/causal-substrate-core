% N3 WEN_COMPLEMENT --- short perspective paper of the FRONTEIRA_COMPACTA line.
...
\title{Why Lorentz-invariant causal sets cannot support an emergent $U(1)$:\\
a causal complement to string-net theory}
...
substrate has ever produced an emergent photon. We argue these two facts have a single
...
$100\%$ electric). The exact boundary between the two worlds is the \emph{definiteness of
...
This perspective argues the non-meeting is structural, states the theorem that separates
the two worlds, and locates the boundary exactly: it is the \emph{signature of the
metric}, not the discreteness, not the dimension, and not a lack of ingenuity.
...
sub-mean-field 4-cycles---tree-like, no finite loop structure to condense.
...
measure} for genuinely dynamical geometry---the one flank our kinematic theorem does
not cover, and where our strongest constraint is only measured (mean-field behaviour in
every CDT-like ensemble tested), not proven; (iii) \emph{import the gauge sector}---the
...
the loop side of the binary (Layer 2) is a measured pattern across seven substrate
families, with the combinatorial theorem (``no invariant random order carries
finite-dimensional loop structure'') still unproven; dynamical geometry beyond the
Poisson measure is constrained only empirically. Within the class, the conclusion is
closed: the absence of an emergent photon on Lorentz-invariant causal sets is not an
open problem awaiting a cleverer construction. It is the signature, doing to gauge
structure exactly what it does to valence, to criticality, and---we now know---to the
entropy of matter across a horizon.
